...
void compute_tensor_smoothed_radial_velocity_fields(const std::vector<double> &observedRedshiftVelocities, const std::vector<double> &observedThetaCoordinates, const std::vector<double> &observedPhiCoordinates, const std::vector<double> &observedDistanceModuli, const std::vector<double> &observedDistanceModulusErrors,
                                                    ...
                                                    SphericalGridFunction &smoothObservedRadialVelocityField, SphericalGridFunction &smoothReconstructedRadialVelocityField, SphericalGridFunction &adaptiveSmoothingScales,
                                                    const std::size_t minNeighbourNumber)
{
    smoothObservedRadialVelocityField = SphericalGridFunction(maxRadius, radialBinNumber, thetaBinNumber, phiBinNumber);
    smoothReconstructedRadialVelocityField = smoothObservedRadialVelocityField;
    adaptiveSmoothingScales = SphericalGridFunction(smoothObservedRadialVelocityField, [&](double value) { return minSmoothingScale; });
    ...
    const ObjectGrid galaxyGroupGrid(gridLength, gridBinNumber,
                                     0.0, 0.0, 0.0,
                                     xCoordinates, yCoordinates, zCoordinates,
                                     false);
    ...
    smoothReconstructedRadialVelocityField.evaluate_for_all_grid_points(
        [&](std::size_t radialBin, std::size_t thetaBin, std::size_t phiBin) {
            const double radius1 = smoothReconstructedRadialVelocityField.radial_coordinate(radialBin);
            const double theta1 = smoothReconstructedRadialVelocityField.theta_coordinate(thetaBin);
            const double phi1 = smoothReconstructedRadialVelocityField.phi_coordinate(phiBin);
            ...
            if (radius1 == 0)
            {
                gsl_vector_set(r1, 0, 0.0);
                gsl_vector_set(r1, 1, 0.0);
                gsl_vector_set(r1, 2, 0.0);
            }
            else
            {
                gsl_vector_set(r1, 0, x1 / radius1);
                gsl_vector_set(r1, 1, y1 / radius1);
                gsl_vector_set(r1, 2, z1 / radius1);
            }

            std::size_t neighbourNumber = 0;

            galaxyGroupGrid.evaluate_for_all_objects_within_distance(
                x1, y1, z1, searchDistance,
                [&](double x12, double y12, double z12, double distance12, std::size_t i_g2) {
                    const double weight2 = windowFunctionWeights[i_g2];
                    const double W12 = window_function(distance12, weight2, minSmoothingScale);
            ...
            if (neighbourNumber < minNeighbourNumber)
            {
                ...
                adaptiveSmoothingScales.value(radialBin, thetaBin, phiBin) = adaptiveSmoothingScale;

                galaxyGroupGrid.evaluate_for_all_objects_within_distance(
                    x1, y1, z1, adaptiveSearchDistance,
                    [&](double x12, double y12, double z12, double distance12, std::size_t i_g2) {
                        const double weight2 = windowFunctionWeights[i_g2];
                        const double W12 = window_function(distance12, weight2, adaptiveSmoothingScale);
            ...
            gsl_blas_dsymv(CblasLower, 1.0, A, R_obs, 0.0, aux);
            gsl_blas_ddot(r1, aux, &smoothObservedRadialVelocityField.value(radialBin, thetaBin, phiBin));

            gsl_blas_dsymv(CblasLower, 1.0, A, R_rec, 0.0, aux);
            gsl_blas_ddot(r1, aux, &smoothReconstructedRadialVelocityField.value(radialBin, thetaBin, phiBin));
